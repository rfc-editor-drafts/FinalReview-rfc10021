...
Furthermore, applications MAY define policies to delete (long-)unused Recipient Contexts and reduce the impact on storage space and check with the Group Manager that an authentication credential with the public key included therein is currently the one associated with a 'kid' value after a number of consecutive failed verifications.
