...
| 2            | Group Flag | Used for a Group OSCORE Security Context; set to 1 if the message is protected with the group mode | RFC 10021 |
...
* CoAP group, i.e., a set of CoAP endpoints where each endpoint is configured to receive one-to-many CoAP requests, e.g., sent to the group's associated IP multicast address and UDP port as defined in {{PRE-RFC10020}}. An endpoint may be a member of multiple CoAP groups. Between application groups and CoAP groups, there can be a many-to-many, one-to-many, many-to-one, or one-to-one relationship. Note that a device sending a CoAP request to a CoAP group is not necessarily a member of that group; it is a member only if it also has a CoAP server endpoint listening for requests sent to the group's associated IP multicast address and UDP port. In order to provide secure group communication, all members of a CoAP group as well as all further endpoints configured only as clients sending CoAP (multicast) requests to the CoAP group have to be a member of a security group. Between security groups and CoAP groups, there can be a many-to-many, one-to-many, many-to-one, or one-to-one relationship.
