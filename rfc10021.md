...
Since they do not join a group as members, external signature checkers need to retrieve the authentication credentials of group members and other selected group data, such as the current Signature Encryption Key, from the Group Manager (see {{ssec-common-context-group-enc-key}}). {{sec-additional-entities}} describes how the Group Manager supports signature checkers.
