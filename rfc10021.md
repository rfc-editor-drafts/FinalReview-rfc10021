...
If it is not feasible or practically possible to store and maintain up-to-date the varying part in non-volatile memory (e.g., due to limited number of write operations), the endpoint MUST be able to detect a loss of the varying part of the Security Context, prevent the reuse of a nonce with the same key, and handle incoming replayed messages.
