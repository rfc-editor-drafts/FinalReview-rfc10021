...
This makes particular sense when the recipient is a client, that would hence be able to process incoming responses protected with the old retained Security Context used to protect the associated request. If, as typically expected, the old Gid is not included in the response, the client will first fail to process the response using the latest Security Context and then use the old retained Security Context as a second attempt.
